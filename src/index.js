...
.stat span{color:#888;font-size:.75rem}
input{width:100%;max-width:360px;background:#141418;border:1px solid #333;border-radius:8px;color:#fff;padding:.5rem .75rem;margin:.5rem 0 1rem}
.wrap{overflow:auto;border:1px solid #222;border-radius:10px}
table{width:100%;border-collapse:collapse;font-size:.85rem}
th,td{padding:.55rem .7rem;border-bottom:1px solid #1c1c1c;text-align:left;white-space:nowrap}
th{background:#1a1a20;color:#888;position:sticky;top:0}
tr:nth-child(even){background:#121214}
.mu{color:#888}.city{font-weight:600}
.badge{border:1px solid #333;border-radius:999px;padding:.1rem .4rem;font-size:.75rem;color:#aaa}
...
<main>
  ...
  <div class="wrap">
    <table>
      <thead>
        <tr>
          <th>#</th><th>北京时间</th><th>格林威治时间</th>
          <th>城市</th><th>地区</th><th>国家</th><th>坐标</th>
        </tr>
      </thead>
      <tbody id="b">
        <tr><td colspan="7" class="mu" style="text-align:center;padding:2rem">加载中…</td></tr>
      </tbody>
    </table>
  </div>
  <footer>数据来自 IP 地理库，可能有误差。不展示原始 IP。</footer>
</main>
<script>
(function(){
  ...
  function draw(list){
    ...
    B.innerHTML=list.map(function(c,i){
      ...
      return '<tr><td class="mu">'+(i+1)+'</td><td>'+e(c.timeBeijing||'—')+'</td><td class="mu">'+e(c.timeGMT||'—')+'</td><td class="city">'+e(c.city||'Unknown')+'</td><td>'+e(c.region||'—')+'</td><td><span class="badge">'+e(c.countryCode||c.country||'—')+'</span></td><td class="mu">'+la+', '+ln+'</td></tr>';
    }).join('');
  }
  function filter(){
    var q=(Q.value||'').toLowerCase().trim();
    draw(!q?all:all.filter(function(c){return (o(c)+' '+(c.timeBeijing||'')+' '+(c.timeGMT||'')).toLowerCase().indexOf(q)>=0}));
  }
  function load(){
    fetch('/cities',{cache:'no-store'})
      .then(function(r){if(!r.ok)throw new Error('HTTP '+r.status);return r.json()})
      .then(function(d){all=d.cities||[];filter()})
      .catch(function(err){B.innerHTML='<tr><td colspan="7" class="err">加载失败：'+e(err.message)+'</td></tr>'});
  }
